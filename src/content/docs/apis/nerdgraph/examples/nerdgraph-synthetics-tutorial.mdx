...
<CollapserGroup>
  ...
  <Collapser
    id="period-attribute"
    title="Define runtime fields"
  >
    The `RUNTIME` attribute defines the attributes of runtime for your non-ping monitors to execute in. These values are not required, but if they are not included the API will attempt to create a legacy runtime monitor.

    For [simple browser monitors](#create-simple-browser-runtime):

      * `RUNTIME_TYPE`: The runtime type used by your monitor. "CHROME_BROWSER" is the only accepted value (required).

      * `RUNTIME_TYPE_VERSION`: The runtime type version used by your monitor. "100" is the only accepted value (required).


    For [scripted API monitors](#create-scripted-API-runtime):
    ...
    For [scripted browser monitors](#create-scripted-browser-runtime):
    ...
    For [step monitors](#create-step-runtime):
    ...
    For [certificate check monitors](#create-certificate-check-runtime):
    ...
    For [broken link monitors](#create-broken-links-runtime):
  ...
  >
    Some fields are for more advanced configurations. These are optional unless otherwise indicated.

    For [ping monitors](#create-ping-options), these include:
    ...
    For [simple browser monitors](#create-simple-browser-options), these include:
    ...
    For [scripted API monitors](#create-scripted-API-runtime):
      ...
      * `SCRIPT_CONTENT`: The actual contents of the script. This should **not** be based64 encoded (required).
    ...
    For [certificate check monitors](#create-certificate-check-runtime):
      ...
      * `DAYS_UNTIL_EXPIRATION`: Notifies you when you need to update your certificate. For example, if you wanted a notification 30 days before the certificate expires, you would input 30 (required).
